...
var import_ecdsa_secp256k1_signature_2019 = require("@bloomprotocol/ecdsa-secp256k1-signature-2019");
...
function base64UrlEncode(unencoded) {
  return import_buffer.Buffer.from(unencoded).toString("base64").replace("+", "-").replace("/", "_").replace(/=+$/, "");
}
function base64UrlDecode(encoded) {
  ...
  return import_buffer.Buffer.from(encoded, "base64").toString("utf8");
}
...
var privateKeyToDoc = async (privateKey, didMethod = "key") => {
  let publicKey, did;
  const privateKeyBuffer = new Uint8Array(import_buffer.Buffer.from(privateKey, "hex"));
  if (didMethod == "ethr") {
    ...
    publicKey = import_buffer.Buffer.from(publicKeyBufferCompressed).toString("hex");
    const addressBuffer = import_buffer.Buffer.from((0, import_keccak256.default)(import_buffer.Buffer.from(publicKeyBuffer.slice(1)))).slice(
      -20
    );
...
async function getVerificationKey({
  ...
  let key = null;
  if (didMethod === "key") {
    key = await import_ed25519_signature_2018.Ed25519VerificationKey2018.generate({
      secureRandom: () => {
        return import_buffer.Buffer.from(seed, "hex");
      }
    });
  } else if (didMethod === "ethr" || didMethod === "moon") {
    const document = await privateKeyToDoc(seed, didMethod);
    key = await import_ecdsa_secp256k1_verification_key_2019.EcdsaSecp256k1VerificationKey2019.generate({
      seed: new Uint8Array(import_buffer.Buffer.from(seed, "hex")),
...
var getKeyValue = (obj, key) => {
  ...
  for (const k in obj) {
    ...
    if (import_lodash.default.isObject(obj[k]) && !import_lodash.default.isArray(obj[k])) {
...
var createMask = ({ mask = {}, credentialSubject = {}, holderPublicKey }) => {
  let maskedClaims = {};
  let maskedMasks = {};
  if (import_lodash2.default.isArray(credentialSubject)) {
    maskedClaims = [];
      ...
      if (maskValues) {
        if (credentialSubject == null ? void 0 : credentialSubject[key]) {
          try {
            ...
            if (import_lodash2.default.isObject(mask == null ? void 0 : mask[key]) && import_lodash2.default.isObject(credentialSubject[key])) {
              ...
              if (import_lodash2.default.size(result == null ? void 0 : result.maskedMasks)) maskedMasks[key] = result == null ? void 0 : result.maskedMasks;
              continue;
            }
...
${error.message}`);
  ...
  } else if (import_lodash2.default.isObject(credentialSubject)) {
        ...
        if (credentialSubject == null ? void 0 : credentialSubject[key]) {
          try {
            if (import_lodash2.default.isObject(mask == null ? void 0 : mask[key]) && import_lodash2.default.isObject(credentialSubject[key])) {
              ...
              if (import_lodash2.default.size(result == null ? void 0 : result.maskedMasks))
...
var fullMask = ({ mask = {}, credentialSubject = {}, holderPublicKey }) => {
  ...
  if (import_lodash2.default.isArray(credentialSubject)) {
    maskedClaims = [];
    for (let key = 0; key < credentialSubject.length; key++) {
      const maskValues = !(mask == null ? void 0 : mask[key]) || import_lodash2.default.isObject(mask == null ? void 0 : mask[key]);
      if (maskValues) {
        if (credentialSubject == null ? void 0 : credentialSubject[key]) {
          try {
            const maskedKey = key;
...
var secp256k12 = __toESM(require("secp256k1"));
var generateKeySignature = ({ data, privateKey }) => {
  const ed = new utils_default.ed25519();
  const privateKeyBuffer = import_buffer2.Buffer.from(privateKey, "hex");
  const dataString = JSON.stringify(data);
  const dataStringHash = new import_sha.default.sha256().update(dataString).digest("hex");
  const dataStringHashBuffer = import_buffer2.Buffer.from(dataStringHash, "hex");
...
var verifyKeySignature = ({ data, signature, publicKey }) => {
  try {
    ...
    const vcHash = new import_sha.default.sha256().update(vcString).digest("hex");
    const vcHashBuffer = import_buffer2.Buffer.from(vcHash, "hex");
...
var generateEthrSignature = ({ data, privateKey }) => {
  ...
  const dataStringHash = new import_sha.default.sha256().update(dataString).digest("hex");
  const dataStringHashBuffer = import_buffer2.Buffer.from(dataStringHash, "hex");
  const signature = import_buffer2.Buffer.from(
...
var verifyEthrSignature = ({ data, signature, publicKey }) => {
  try {
    ...
    const vcHash = new import_sha.default.sha256().update(vcString).digest("hex");
    const vcHashBuffer = import_buffer2.Buffer.from(vcHash, "hex");
    const dataStringBuffer = new Uint8Array(import_buffer2.Buffer.from(signature, "hex"));
    const publicKeyBuffer = new Uint8Array(import_buffer2.Buffer.from(publicKey, "hex"));
...
var Ed25519 = class {
  constructor() {
    this.ed = new import_elliptic.eddsa("ed25519");
  }
  /**
...
// src/utils/index.ts
var secp256k13 = __toESM(require("secp256k1"));
...
var create = async ({
  ...
  try {
    ...
    if ((_h = issuerDocument == null ? void 0 : issuerDocument.document) == null ? void 0 : _h.verificationMethod) {
      ...
      switch ((_j = verificationMethod[0]) == null ? void 0 : _j.type) {
        case "Ed25519VerificationKey2018":
          ...
          verificationKey = await import_ed25519_signature_20182.Ed25519VerificationKey2018.from({
          ...
          break;
        case "EcdsaSecp256k1VerificationKey2019":
          verificationKey = import_ecdsa_secp256k1_verification_key_20192.EcdsaSecp256k1VerificationKey2019.from({
          ...
          if (!suite)
            suite = new import_ecdsa_secp256k1_signature_2019.EcdsaSecp256k1Signature2019({
              key: verificationKey,
              date: issuanceDate
            });
          break;
    ...
    const result = await import_vc.verifiable.credential.create({
      format: ["vc"],
      credential,
      suite,
      documentLoader
...
var verify = async ({
    ...
    if ((_c = documentLoaderResult == null ? void 0 : documentLoaderResult.document) == null ? void 0 : _c.verificationMethod) {
      ...
      switch ((_e = verificationMethod[0]) == null ? void 0 : _e.type) {
        ...
        case "EcdsaSecp256k1VerificationKey2019":
          ...
          const key = import_ecdsa_secp256k1_verification_key_20192.EcdsaSecp256k1VerificationKey2019.from({
          ...
          suite = new import_ecdsa_secp256k1_signature_2019.EcdsaSecp256k1Signature2019({ key });
          break;
      }
  ...
  if (import_lodash3.default.isObject((_l = (_k = vc == null ? void 0 : vc.credentialSubject) == null ? void 0 : _k.selectiveDisclosureMetaData) == null ? void 0 : _l.mask)) {
  ...
  try {
    return await import_vc.verifiable.credential.verify({
      credential: vc,
      format: ["vc"],
      documentLoader,
      suite
...
var import_ecdsa_secp256k1_signature_20192 = require("@bloomprotocol/ecdsa-secp256k1-signature-2019");
var import_ed25519_signature_20183 = require("@transmute/ed25519-signature-2018");
var import_vc2 = require("@transmute/vc.js");
var import_ecdsa_secp256k1_verification_key_20193 = require("@bloomprotocol/ecdsa-secp256k1-verification-key-2019");
var create2 = async ({
  suite,
  ...
  if (!suite) {
    ...
    if (didMethod === "key") {
      ...
      suite = new import_ecdsa_secp256k1_signature_20192.EcdsaSecp256k1Signature2019({
        key: keyPairIssuer,
        date: issuanceDate
      });
    }
  ...
  const result = await import_vc2.verifiable.presentation.create({
    presentation,
    format: ["vp"],
    documentLoader,
    challenge,
...
var verify2 = async ({
  ...
  if (didMethod === "key") {
    ...
    const key = import_ecdsa_secp256k1_verification_key_20193.EcdsaSecp256k1VerificationKey2019.from({
